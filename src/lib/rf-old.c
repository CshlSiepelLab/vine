...
/* Canonicalize a split, KEEPING leaf edges (unlike bm_canonical) but
 * tie-breaking balanced splits by the side containing taxon 0 (unlike
 * bm_canonical_any), so the same unrooted split maps to the same mask across
 * trees regardless of rooting/traversal.  Returns NULL only for the trivial
 * whole-set / empty split. */
...
/* Like dfs_splits_blen but uses bm_canonical_all (keeps leaf edges and
 * tie-breaks balanced splits). */
