...
TreeNode* upgma_infer(Matrix *initD, char **names, Matrix *dt_dD) {
  int n = initD->nrows;
  int N = 2*n - 2;
  int i, j, u = -1, v = -1, w;
  Matrix *D;
  ...
  if (initD->nrows != initD->ncols || n < 2)
    die("ERROR upgma_infer: bad distance matrix\n");
  ...
  for (i = 0; i < N; i++) {
    if (vec_get(active, i) == TRUE) {
      if (node_u == NULL) {
        ...
        die("ERROR upgma_infer: more than two nodes left at root\n");
    }
  }
...
/* Efficiently compute dL/dD for UPGMA from a finished tree and
   ...
   tree   : rooted ultrametric tree produced by upgma_infer
