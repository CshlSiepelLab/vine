...
int main(int argc, char *argv[]) {
  ...
  double split_kl, pairwise_dist_kl, embed_kl;
  int dim = -1;   /* -1 means "use vine's default formula" */
  int nthreads = 1;
  int pairwise_dist_kl_enabled = FALSE;
  int embed_dist_kl = FALSE;

  struct option long_opts[] = {
    {"pairwise-dist-kl", 0, 0, 'p'},
    {"embed-dist-kl", 0, 0, 'e'},
    {"dim", 1, 0, 'd'},
    {"nthreads", 1, 0, 'n'},
    {"help", 0, 0, 'h'},
  ...
  while ((c = getopt_long(argc, argv, "ped:n:h", long_opts, &opt_idx)) != -1) {
    switch (c) {
    case 'p':
      pairwise_dist_kl_enabled = TRUE;
      break;
    case 'e':
      embed_dist_kl = TRUE;
      break;
  ...
  if (pairwise_dist_kl_enabled) {
    fprintf(stderr, "Computing raw pairwise-distance KL divergence...\n");
    tr_pairwise_dist_kl(trees_est, trees_ref, &pairwise_dist_kl);
  }
  ...
  if (pairwise_dist_kl_enabled)
    printf("Mean_pairwise_dist_KL: %f\n", pairwise_dist_kl);
  if (embed_dist_kl)
